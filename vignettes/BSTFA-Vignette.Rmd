...
Below is an example of code used to plot the posterior mean temperature values (black line) at an observed location, "Alpine, Utah," with 95\% posterior predictive bounds (gray bands), and observed measurements (gray circles). Figure \@ref(fig:fig-location) provides the plot.  Notice that the posterior mean (black line) closely follows the pattern of the observed data, but, as expected due to basis smoothing, is more smooth than the observed data. Increasing the number of bases will decrease the smoothness (but increase computation time).

```{r fig-location, fig.cap="Posterior mean temperature values (black line) at an in-sample location, Alpine, Utah, with 95% posterior predictive bounds (gray bands), and observed temperature measurements (gray circles).", fig.align="center", fig.height=3.5, fig.width=6}
loc = 1 # Alpine, Utah in our data set
plot_location(out,
              location=loc,
              type='mean',
...
fixed_locations <- utahDataList$Coords[out$factors.fixed,] #utahDataList$Coords[c(144,89,129,78),]
